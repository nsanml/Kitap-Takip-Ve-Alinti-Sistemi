...
try {
    $toplamSorgu = $vt->query("SELECT COUNT(*) FROM quotes");
...
<head>
	...
	<title>Kitap Durağı - Alıntılar</title>
	 <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
	 <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
     <script language="javascript" src="../js/alinti.js"></script>
...
<div class="container mt-5 mb-5 feed-container">
    ...
    <?php if (count($alintilar) == 0): ?>
        <div class='alert alert-warning text-center'>Henüz hiç alıntı eklenmemiş.</div>
    <?php endif; ?>
    ...
    <div class="d-flex flex-column align-items-center w-100">
        <?php foreach ($alintilar as $row): ?>
            <?php 
                ...
                $kitapResim = !empty($row['kapak']) ? $row['kapak'] : 'https://via.placeholder.com/150x220?text=Kapak';
            ...
            <div class="modern-card">
                ...
                <div class="m-header">
                    <a href="profil.php?id=<?= $row['user_id'] ?>">
                   	...
                   	<div class="m-user">
                        <a href="profil.php?id=<?= $row['user_id'] ?>" style="text-decoration:none;">
                            <h6><?= $row['username'] ?></h6>
                        </a>
                        <span><?= date("d.m.Y", strtotime($row['olusma_tarih'])) ?></span>
                    </div>
                </div>

                <div class="m-body">
                    <div class="m-quote" style="font-size: 1.1rem; line-height: 1.6;">
                        <?= $row['content'] ?>
                    </div>
                    
                    <a href="kitap_detay.php?id=<?= $row['book_id'] ?>" class="book-badge mt-3 w-100">
                        <img src="../resimler/kitaplar/<?= $kitapResim ?>" alt="Kapak" onerror="this.src='https://via.placeholder.com/60x90?text=Kapak';">
                        <div class="book-info">
                            <span class="book-title"><?= $row['baslik'] ?></span>
                            <span class="book-author"><?= $row['yazar'] ?></span>
                        </div>
                    </a>
                </div>
                ...
                <div class="m-footer">
                    <div class="m-actions">
                        <span class="like-btn <?= $begenilmis ?>" onclick="begen(this, <?= $row['alinti_id'] ?>)">
                            <span class="icon"><?= $kalpIkonu ?></span> 
                            <span class="count" id="like-count-<?= $row['alinti_id'] ?>"><?= $row['like_count'] ?></span>
                        </span>
                    </div>
                </div>
    ...
    <?php if ($toplamSayfa > 1): ?>
        <nav aria-label="Sayfalama" class="mt-4 mb-5">
            <ul class="pagination justify-content-center">
                <li class="page-item <?= ($sayfa <= 1) ? 'disabled' : '' ?>"><a class="page-link" href="?sayfa=<?= $sayfa - 1 ?>">‹</a></li>
                <?php for($s = 1; $s <= $toplamSayfa; $s++): ?>
                    <li class="page-item <?= ($s == $sayfa) ? 'active' : '' ?>"><a class="page-link" href="?sayfa=<?= $s ?>"><?= $s ?></a></li>
                <?php endfor; ?>
                <li class="page-item <?= ($sayfa >= $toplamSayfa) ? 'disabled' : '' ?>"><a class="page-link" href="?sayfa=<?= $sayfa + 1 ?>">›</a></li>
            </ul>
        </nav>
    <?php endif; ?>
